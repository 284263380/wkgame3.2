...
wkcsv::~wkcsv()
{
    delete[] m_columns;
    m_cntColumn = 0;
    for (int i=0;i<(int)m_lstDataLine.size();i++)
    {
        wkcsvItem* v = m_lstDataLine[i];
        if(!v)
            continue;
...
void  wkcsv::removeTail(char* buff)
{
    if(!buff)
        return;
    int len = strlen(buff);
...
void wkcsv::analyse(char* buff,list<char*>& lstItem)
{
    ...
    for (int i=0;i<len;i++)
    {
        ...
        else
        {
            ...
            {
                buff[pos]=buff[i];
                if( (buff[i] ==','|| buff[i] == '\0') && numQuotation%2==0)
                {
                    buff[i]='\0';
                    buff[pos]='\0';
                    break;
